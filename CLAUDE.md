...
- **Manifest-gated sidecar consumption.** `parse_corpus` skips a `.vtt`/`.srt` (as `consumed-by-video`) only when its same-stem video already has a `method: "video-lane"` entry in the out dir's manifest whose `md` exists — there is no flag, so a corpus that never ran the video lane parses byte-identically. `assemble` writes that consumed entry and deletes the sidecar's stale parsed doc in the same step. The entry is what lets `brain_sync` retire the old transcript doc — and only while the video doc exists.
