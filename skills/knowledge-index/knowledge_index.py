...
def cmd_index(a):
    ...
    c.executescript("""
      CREATE TABLE IF NOT EXISTS chunks(id INTEGER PRIMARY KEY, source TEXT, ord INT, text TEXT);
    ...
    for f in sorted(glob.glob(os.path.join(a.corpus, "**", "*.md"), recursive=True)):
        ...
        for i, ch in enumerate(chunk_md(open(f).read(), a.max_chars)):
            rows.append((src, i, ch))
    print(f"chunks: {len(rows)} from {len(set(r[0] for r in rows))} docs; embedding ({a.model})…", file=sys.stderr)
    vecs = embed(a.model, [r[2] for r in rows])
    for (src, i, ch), v in zip(rows, vecs):
        rid = c.execute("INSERT INTO chunks(source,ord,text) VALUES(?,?,?)", (src, i, ch)).lastrowid
        c.execute("INSERT INTO chunks_fts(rowid,text) VALUES(?,?)", (rid, ch))
...
def search(c, model, query, k):
    ...
    out = []
    for rid in top:
        src, txt = c.execute("SELECT source,text FROM chunks WHERE id=?", (rid,)).fetchone()
        out.append({"score": round(score[rid], 5), "source": src, "text": txt})
