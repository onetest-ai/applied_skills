...
The corpus or the parse shifts (e.g. `visual-parse` now transcribes diagrams, surfacing concepts that were invisible before), so the taxonomy under-covers — the signal is a rising share of **untagged** chunks. Grow it **additively** (never rename/remove — node ids are `slug(label)`; renaming orphans every tag/edge):
