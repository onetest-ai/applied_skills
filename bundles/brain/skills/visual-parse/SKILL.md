---
name: visual-parse
description: Use when a corpus has slide decks / diagram-heavy pages (flows, timelines, circle process charts, complex tables) that a text extractor mangles, or meeting recordings whose shared screens carry context a transcript alone would lose. Renders each page to an image, flags the visual ones, has a vision-capable Sonnet model transcribe them to faithful structured Markdown, and extracts real table grids deterministically. Two representations per page — a semantic transcription (for retrieval) and the full factual content (image + verbatim text + table cells, for answering). Feeds the knowledge-index.
---
...
doc (pdf / pptx via soffice→pdf)
  ...
  → 🤖 vision agents (Sonnet, VLM)      transcribe flagged pages → result_<k>.json {img_sha: markdown}
