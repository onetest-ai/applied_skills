...
Author a brief, cited memo on **$subject**. The Brain contract below governs citation and
sourcing; `../_shared/authoring.md` holds the full pipeline write-up and worked examples.
...
5. **Human gate.** Show the draft memo, the verifier verdict, and the proposed path (`docs/kb/<slug>.md`) to the human. Do **not** write to disk without explicit approval.
