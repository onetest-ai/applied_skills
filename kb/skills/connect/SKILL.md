...
allowed-tools: mcp__brain__health mcp__plugin_applied-skills_brain__health Bash(./brain *) Read
---

Detect and (if needed) help the user register the Brain that `kb` grounds itself in.

1. **Detect the Brain.** Call `health` in each namespace kb knows about — `mcp__brain__health` and `mcp__plugin_applied-skills_brain__health` — since the same server can be mounted under either name depending on how it was registered.
