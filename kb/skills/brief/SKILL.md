...
1. **Gather.** Call the Brain tools to collect narrative, metrics, and relations on the subject. Route per doctrine (narrative → `search_knowledge`, numbers → `get_metric`, relations → `get_taxonomy`, cited sections → `get_evidence`).

2. **Draft.** Compose a 200–400-word memo with inline citation tags (`[RAG:id]`, `[MART:metric@grain]`, `[GRAPH:node]`). Mark any unsupported claim as "Not modeled: …".
...
4. **Propose.** Show the draft memo, the verifier verdict, and the proposed path (`docs/kb/<slug>.md`) to the human. Do **not** write yet — await approval.
